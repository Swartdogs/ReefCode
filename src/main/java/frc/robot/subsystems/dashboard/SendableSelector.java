...
/**
 * The {@link SendableSelector} class is a useful tool for presenting a selection of options to the
 * {@link SmartDashboard}. This class is adapted from the {@link SendableChooser} class provided by
 * the WPILibrary. The adaptations made are intended to provide a way to programatically change
 * available and selected options from the robot code, instead of only allowing external applications
 * to select options.
 *
